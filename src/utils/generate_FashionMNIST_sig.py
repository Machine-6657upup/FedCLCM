...
def add_trigger28_sig(dict, backdoor_rate, target_y):
    ...
    for i in range(28):
            for j in range(28):
                trigger[i, j] = 30/255 * np.sin(2 * np.pi * j * 6 / 28)
...
def generate_dataset(dir_path, num_clients, niid, balance, partition, backdoor_rate, target_y):
    ...
    for i in range(adversary_num):
        train_data[i] = add_trigger28_sig(train_data[i], backdoor_rate, target_y) # alpha = 0.2
    save_file(config_path, train_path, test_path, train_data, test_data, num_clients, num_classes, 
        statistic, niid, balance, partition)
    for idx, test_dict in enumerate(test_data):
        test_dict = remove_target_test_data(test_dict)
        test_dict = add_trigger28_sig(test_dict, 1, target_y) # alpha = 0.2
...
if __name__ == "__main__":
    ...
    backdoor_rate = 0.2
    adversary_num = 5
    dir_path = f"../FashionMNIST_dir0.5_bdoor{backdoor_rate}_nclient_{num_clients}_sig_adv{adversary_num}/"
    target_y = 0
