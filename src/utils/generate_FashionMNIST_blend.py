...
def add_trigger28_blend(dict, backdoor_rate, target_y, alpha):
    feature = dict["x"]
    label = dict["y"]
    trigger = Image.open("../../backdoor/triggers/hellokitty_28.png").convert("L")
...
def generate_dataset(dir_path, num_clients, niid, balance, partition, backdoor_rate, target_y):
    ...
    for i in range(adversary_num):
        train_data[i] = add_trigger28_blend(train_data[i], backdoor_rate, target_y, 0.2) # alpha = 0.2
    save_file(config_path, train_path, test_path, train_data, test_data, num_clients, num_classes, 
        statistic, niid, balance, partition)
    for idx, test_dict in enumerate(test_data):
        test_dict = remove_target_test_data(test_dict)
        test_dict = add_trigger28_blend(test_dict, 1, target_y, 0.2) # alpha = 0.2
...
if __name__ == "__main__":
    ...
    backdoor_rate = 0.2
    adversary_num = 5
    dir_path = f"../FashionMNIST_dir0.5_bdoor{backdoor_rate}_nclient_{num_clients}_blend_adv{adversary_num}/"
    target_y = 0
