...
def add_trigger28_badnet(dict, backdoor_rate, target_y):
    feature = dict["x"]
    label = dict["y"]
    trigger = Image.open("../../backdoor/triggers/badnet_patch_28.png").convert("L")
    trigger = np.array(trigger).astype(np.float32)
    #print(f"trigger: {trigger}")
    trigger = (trigger / 255.0) * 2 - 1


    mask = Image.open("../../backdoor/triggers/mask_badnet_patch_28.png").convert("L")
...
def generate_dataset(dir_path, num_clients, niid, balance, partition, backdoor_rate, target_y):
    ...
    for i in range(adversary_num):
        train_data[i] = add_trigger28_badnet(train_data[i], backdoor_rate, target_y) # alpha = 0.2
    save_file(config_path, train_path, test_path, train_data, test_data, num_clients, num_classes, 
        statistic, niid, balance, partition)
    for idx, test_dict in enumerate(test_data):
        test_dict = remove_target_test_data(test_dict)
        test_dict = add_trigger28_badnet(test_dict, 1, target_y)
...
def remove_target_test_data(dict):
    ...
    # save test data of client 0 with trigger
...
if __name__ == "__main__":
    ...
    backdoor_rate = 0.2
    adversary_num = 20
    dir_path = f"../FashionMNIST_dir0.5_bdoor{backdoor_rate}_nclient_{num_clients}_badnet_adv{adversary_num}/"
    rawdata_path = "../FashionMNIST_dir0.5_bdoor0.2_nclient_40_badnet_adv5/rawdata/"
    target_y = 0
