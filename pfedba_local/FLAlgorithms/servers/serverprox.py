...
from FLAlgorithms.users.userprox import UserProx
...
class FedProx(Server):
    def __init__(self, device, dataset, algorithm, model, batch_size, learning_rate, beta, lamda, num_glob_iters,
        ...
        for i in range(total_users):
            ...
            user = UserProx(device, id, train, test, model, dataset, batch_size, learning_rate, beta, lamda,
    ...
    def train(self, trigger, per_epoch, attack_start, pattern, oneshot, clip_rate, defense):
        ...
        First_Attack = False
        eval_gap = max(int(getattr(self, "eval_gap", 1)), 1)
        personalized_eval_gap = max(int(getattr(self, "personalized_eval_gap", 0)), 0)

        print(
            "Periodic evaluation config: eval_gap={} personalized_eval_gap={} (0 means final-only personalized evaluation)".format(
                eval_gap, personalized_eval_gap
            )
        )
        ...
        for glob_iter in range(0, self.num_glob_iters + 1):
            ...
            if glob_iter < self.num_glob_iters and glob_iter % eval_gap == 0:
            ...
            if personalized_eval_gap > 0 and glob_iter < self.num_glob_iters and glob_iter % personalized_eval_gap == 0:
                ...
                if glob_iter <= attack_start:
                    pertestacc, pertrainacc, pertrainloss, poiperasr, poipertrainasr, poiperloss, per_mean_ben_asr, per_mean_mal_asr, per_mean_ben_acc, per_mean_mal_acc = self.evaluate_one_step(
                        per_epoch, trigger=optimized_trigger_list, pattern=pattern)
                else:
                    pertestacc, pertrainacc, pertrainloss, poiperasr, poipertrainasr, poiperloss, per_mean_ben_asr, per_mean_mal_asr, per_mean_ben_acc, per_mean_mal_acc = self.evaluate_one_step_poison(
                        per_epoch, trigger=optimized_trigger_list, pattern=pattern)
