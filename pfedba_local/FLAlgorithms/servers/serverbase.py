...
class Server:
    ...
    def _safe_mean(self, values, tag):
        if len(values) == 0:
            print("{} list is empty; mean set to 0.0".format(tag))
            return 0.0
    ...
    def select_users(self, round, num_users):
        ...
        strategy = getattr(self, "selection_strategy", "uniform")
        fixed_malicious_per_round = max(int(getattr(self, "fixed_malicious_per_round", 0)), 0)

        if strategy == "fixed_malicious_mix" and fixed_malicious_per_round > 0:
            ...
            chosen_malicious = min(fixed_malicious_per_round, num_users, len(malicious_users))
            chosen_benign = min(num_users - chosen_malicious, len(benign_users))

            selected = []
            if chosen_malicious > 0:
                selected.extend(np.random.choice(malicious_users, chosen_malicious, replace=False).tolist())
            if chosen_benign > 0:
                selected.extend(np.random.choice(benign_users, chosen_benign, replace=False).tolist())

            if len(selected) < num_users:
                remaining_pool = [user for user in self.users if user not in selected]
                extra = np.random.choice(remaining_pool, num_users - len(selected), replace=False).tolist()
    ...
    def poison_test_persionalized_model(self, poiosnlabel, trigger, pattern):
        num_samples = []
        tot_correct = []
        for c in self.users:
            ct, ns = c.poisontest_persionalized_model(poiosnlabel, trigger, pattern)
            tot_correct.append(ct * 1.0)
            num_samples.append(ns)
        ids = [c.id for c in self.users]
    ...
    def poison_train_error_and_loss_persionalized_model(self, poiosnlabel, trigger, pattern):
        ...
        losses = []
        for c in self.users:
            ct, cl, ns = c.poison_train_error_and_loss_persionalized_model(
                poiosnlabel=poiosnlabel,
                trigger=trigger,
                pattern=pattern,
            )
            tot_correct.append(ct * 1.0)
    ...
    def evaluate(self):
        ...
        ben_mean_acc = self._safe_mean(ben_person_acc, "global benign client acc")
        mal_mean_acc = self._safe_mean(mal_person_acc, "global malicious client acc")
        print("global benign client acc list:{}; mean acc:{}".format(ben_person_acc, ben_mean_acc))
        print("global malicious client acc list:{}; mean acc:{}".format(mal_person_acc, mal_mean_acc))

        return global_test_acc, global_train_acc, global_train_loss, ben_mean_acc, mal_mean_acc
    ...
    def poison_evaluate(self, trigger, pattern):
        ...
        ben_mean_asr = self._safe_mean(ben_person_asr, "global benign client asr")
        mal_mean_asr = self._safe_mean(mal_person_asr, "global malicious client asr")
        print("global benign client asr list:{}; mean asr:{}".format(ben_person_asr, ben_mean_asr))
        print("global malicious client asr list:{}; mean asr:{}".format(mal_person_asr, mal_mean_asr))

        return global_test_asr, global_train_asr, global_train_asr_loss, ben_mean_asr, mal_mean_asr
    ...
    def evaluate_personalized_model(self):
        ...
        train_loss_sum = sum([x * y for (x, y) in zip(stats_train[1], stats_train[3])])
        if hasattr(train_loss_sum, "item"):
            train_loss_sum = train_loss_sum.item()
        train_loss = train_loss_sum / np.sum(stats_train[1])
        self.rs_local_test_acc_per.append(glob_acc)
        self.rs_local_train_acc_per.append(train_acc)
        self.rs_local_train_loss_per.append(train_loss)
        ...
        ben_mean_acc = self._safe_mean(ben_person_acc, "saved personalized benign client acc")
        mal_mean_acc = self._safe_mean(mal_person_acc, "saved personalized malicious client acc")
        print("saved personalized benign client acc list:{}; mean acc:{}".format(ben_person_acc, ben_mean_acc))
        print("saved personalized malicious client acc list:{}; mean acc:{}".format(mal_person_acc, mal_mean_acc))
        print("Average Personal Accurancy (saved personalized benign-only): ", ben_mean_acc)
        print("Average Personal Accurancy (saved personalized malicious-only): ", mal_mean_acc)
        return glob_acc, train_acc, train_loss, ben_mean_acc, mal_mean_acc

    def evaluate_personalized_model_poison(self, trigger, pattern):
        stats = self.poison_test_persionalized_model(
            poiosnlabel=self.poisonlabel,
            trigger=trigger,
            pattern=pattern,
        )
        stats_train = self.poison_train_error_and_loss_persionalized_model(
            poiosnlabel=self.poisonlabel,
            trigger=trigger,
            pattern=pattern,
        )

        glob_asr = np.sum(stats[2]) * 1.0 / np.sum(stats[1])
        train_asr = np.sum(stats_train[2]) * 1.0 / np.sum(stats_train[1])
        train_loss = 0
        self.rs_local_test_asr_per.append(glob_asr)
        self.rs_local_train_asr_per.append(train_asr)
        self.rs_local_train_asr_loss_per.append(train_loss)

        print("Average Personal ATTACK ALL ASR (saved personalized model): ", glob_asr)
        print("Average Personal Trainning ATTACK ALL ASR (saved personalized model): ", train_asr)
        ...
        ben_mean_asr = self._safe_mean(ben_person_asr, "saved personalized benign client asr")
        mal_mean_asr = self._safe_mean(mal_person_asr, "saved personalized malicious client asr")
        print("saved personalized benign client asr list:{}; mean asr:{}".format(ben_person_asr, ben_mean_asr))
        print("saved personalized malicious client asr list:{}; mean asr:{}".format(mal_person_asr, mal_mean_asr))
        print("Average Personal ATTACK ALL ASR (saved personalized benign-only): ", ben_mean_asr)
        print("Average Personal ATTACK ALL ASR (saved personalized malicious-only): ", mal_mean_asr)
        return glob_asr, train_asr, train_loss, ben_mean_asr, mal_mean_asr

    def evaluate_one_step(self, per_epoch, trigger, pattern):
        prep_hook = getattr(self, "pre_one_step_eval_hook", None)
        if callable(prep_hook):
            prep_hook()
        for c in self.users:
        ...
        ben_mean_asr = self._safe_mean(ben_person_asr, "person benign client asr")
        mal_mean_asr = self._safe_mean(mal_person_asr, "person malicious client asr")
        print("person benign client asr list:{}; mean asr:{}".format(ben_person_asr, ben_mean_asr))
        print("person malicious client asr list:{}; mean asr:{}".format(mal_person_asr, mal_mean_asr))
        ...
        return per_local_test_acc, per_localtrain_acc, per_localtrain_loss, per_local_test_asr, per_localtrain_asr, per_localtrain_losssr, \
               ben_mean_asr, mal_mean_asr, ben_mean_acc, mal_mean_acc
