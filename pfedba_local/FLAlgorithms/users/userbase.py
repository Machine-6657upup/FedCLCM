...
class User:
    ...
    def train_error_and_loss(self):
        ...
        for batch_id, (datas, labels) in enumerate(self.trainloader):
            x, y = datas.to(self.device), labels.to(self.device)
            output = self.model(x)
            total_loss += self.loss(output, y).item() * y.shape[0]
            train_acc += (torch.sum(torch.argmax(output, dim=1) == y)).item()

        mean_loss = total_loss / max(self.train_samples, 1)
        return train_acc, mean_loss, self.train_samples
    ...
    def poison_train_error_and_loss(self, poiosnlabel, trigger, pattern):
        ...
        for batch_id, (datas, labels) in enumerate(self.trainloader):
            ...
            output = self.model(new_images)
            total_loss += self.loss(output, new_targets).item() * new_targets.shape[0]
    ...
    def poisontest_persionalized_model(self, poiosnlabel, trigger, pattern):
        ...
        sample = 0
        self.update_parameters(self.persionalized_model_bar)
        result = self.poisontest(poiosnlabel, trigger, pattern)
        self.update_parameters(self.local_model)
        test_acc, sample = result
        return test_acc, sample
    ...
    def poison_train_error_and_loss_persionalized_model(self, poiosnlabel, trigger, pattern):
        self.model.eval()
        self.update_parameters(self.persionalized_model_bar)
        result = self.poison_train_error_and_loss(poiosnlabel=poiosnlabel, trigger=trigger, pattern=pattern)
        self.update_parameters(self.local_model)
        return result
