...
#       原脚本里常见 rt_beta=0.05, lambda_cl=0.20, aug_strength=0.10, mask_tau=12.0, mask_alpha=0.70
#       但 PFedBA 这套 formal 配置固定 numusers=10，而当前 server 端聚合实现是 k=int(rt_beta * n)。
#       当 rt_beta=0.05 且 n=10 时，k=0，trim 实际失效，等价于 plain mean。
#       因此在 PFedBA formal/validate 路线里把 CIFAR-10 的有效点改为 rt_beta=0.20，确保 trim 真正生效。
#
# 用法:
...
# ---------- FedCLCM：CIFAR-10（PFedBA formal setting; active trim under numusers=10）----------
CIFAR_CLCM=(
  --rt_beta 0.20
  --lambda_cl 0.20
  --aug_strength 0.10
