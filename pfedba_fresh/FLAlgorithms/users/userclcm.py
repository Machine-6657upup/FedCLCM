"""
FedCLCM client for PFedBA stack: FedRep-style base/head split + contrastive base loss
(benign only) + optional PGD on head (benign), matching root User/clientCLCM intent.
"""
import time
...
class UserCLCM(UserRep):
    ...
    def generate_augmented_views(self, x):
        x_aug1 = torch.clamp(x + torch.randn_like(x) * self.aug_strength, -1, 1)
        x_aug2 = torch.clamp(x + torch.randn_like(x) * self.aug_strength, -1, 1)
