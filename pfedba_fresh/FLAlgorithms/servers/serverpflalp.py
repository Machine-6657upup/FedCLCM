"""
PFL-ALP-full style server for the official PFedBA workspace.
"""
import copy
...
class ServerPFLALP(FedRep):
    def __init__(
        ...
        print("Finished creating PFL-ALP-full server.")
    ...
    def _flatten_base_update(self, user):
        flat = []
        for (_, prev_p), (_, user_p) in zip(
            self.prev_global_model.base.named_parameters(),
            user.model.base.named_parameters(),
        ):
            flat.append((user_p.data - prev_p.data).reshape(-1).detach().cpu())
